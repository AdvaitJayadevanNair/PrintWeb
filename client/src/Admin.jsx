...
export default function Admin({ db, storage, auth }) {
    ...
    useEffect(() => {
        ...
        const q2 = query(doc(db, 'options', 'options'));
        ...
        const unsubscribe2 = onSnapshot(q2, (doc) => {
            setOptions(doc.data());
        });
        return () => {
            unsubscribe1();
        ...
        <>
            <section className="section">
                ...
                <h2 className="subtitle">
                    Current printer: <strong className={options.deviceId ? '' : 'has-text-danger'}>{options.deviceId ? options.deviceId : 'Not Set'}</strong>
                </h2>
                ...
                <div className="columns">
                                ...
                                {docs.map((doc, index) => {
                                    return (
                                        <tr key={index}>
                                            ...
                                            <td>
                                                <a href="#" title={doc.fileName} onClick={() => redirectToFile(doc.filePath)}>
                                                    {doc.fileName}({humanFileSize(doc.fileSize)})
                                                </a>
                                            </td>
                    ...
                    <div className="column">
                        <div className="box">
                            ...
                            <button className="button is-primary" disabled={disabled} onClick={changePrinter}>
                                Change
                            </button>
                        </div>
                        <div className="box">
                            <div className="field">
                                ...
                                <button className="button is-danger" disabled={disabled} onClick={clearLog}>
                                    Clear Log
                                </button>
                            </div>
                        </div>
